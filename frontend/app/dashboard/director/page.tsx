...
export default function DirectorDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "attendance" | "incidents" | "classes"
  >("overview");
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      if (reports) {
        ...
        const classStatsArray = Array.from(classMap.values())
          .map((cls) => ({
            ...cls,
            avgAttendance:
              cls.totalReports > 0
        ...
        const totalHealth = reports.filter(
          (r: any) => r.health_incident,
        ).length;
        const totalDiscipline = reports.filter(
          (r: any) => r.discipline_issue,
        ).length;
        const totalParent = reports.filter(
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-600">
                    {overallStats.disciplineIncidents}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {overallStats.parentCommunications}
                  </p>
                </div>
                  ...
                  >
                    {classStats.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          entry.avgAttendance >= 90
                            ? "#10B981"
                            : entry.avgAttendance >= 75
                              ? "#FBBF24"
                              : "#EF4444"
                        }
                      />
